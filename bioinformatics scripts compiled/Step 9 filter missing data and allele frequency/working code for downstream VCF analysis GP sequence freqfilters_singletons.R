...
vcf <- read.vcfR("GP_filtered_step2_DPdiff5x.vcf")
...
length(which(read.depth.ref.avg >= 3))
length(which(read.depth.snp.avg >= 3))
...
length(which(coverage.rd2$diff >= 80))
# is already done so pushed to 90 
...
# filter 90% loc
# note 100% of loc have a callrate of 70%
#sum(snps.rd2$callrate >70)
sum(snps.rd2$callrate >90)
...
# this one is fine. 
snps.rd2 <- subset(snps.rd2, snps.rd2$callrate>=70)
...
# Function calls and subsets don't work well on row by row comparisons. Tried a few things but nothing worked to address this
# So I decided to transpose the data do the filter and transpose back. It works fine.
snps.rd3 <- t(snps.rd3)
...
snps.rd3 <- subset(snps.rd3, snps.rd3[,collen] >= 50)
...
# To remove singletons we need to know the number of inds 
ncol(snps.rd4)
ncol(snps)
# 518 rows, 6 are details so that makes 512 
# We want a maf to be larger than 1 count but less than 2. 
#1/(565*2) = 0.0008849558
#2/(565*2) = 0.001769912
# There are likely to be inds that are missing loci (we can say about 20% so we don't want to put the value just around 1)
# we will do the middle ground of the two = 0.001327434
min(snps.rd4$maf)
length(which(snps.rd4$maf >= 0.0008849558))
length(which(snps.rd4$maf >= 0.001327434))
length(which(snps.rd4$maf >= 0))
#```
# so 0.001464844 works perfectly, I've tested around the value and nothing gets added until 0.11 and nothing is lost until closer to 0.0023
...
snps.rd5 <-subset(snps.rd4,snps.rd4$maf >= 0.001327434)
...
colnames(vcf@gt)
colnames(vcf2@gt)
setwd("D:/Dropbox/RA/Kat/GP bioinformatics/Filtering_steps/3 filter frequency differences and missing data")

# This appeared to have worked!
# And the writing of the vcf worked to!
?write.vcf()
write.vcf(vcf2, file = "GP_filtered_step3_call_loc60_ind50_maf_singleton.vcf.gz")
