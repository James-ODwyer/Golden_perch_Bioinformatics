...
# this one is fine. 
snps.rd2 <- subset(snps.rd2, snps.rd2$callrate>=90)
...
snps.rd3 <- subset(snps.rd3, snps.rd3[,collen] >= 70)
...
#  ```{r}
length(which(snps.rd4$maf >= 0.01))
#```
...
snps.rd5 <-subset(snps.rd4,snps.rd4$maf >= 0.01)
...
?write.vcf()
write.vcf(vcf2, file = "GP_filtered_step3_call_loc90_ind70_maf1.vcf.gz")
...
# There is one extra column that isn't a loci
#There are 7 extra rows that aren't inds 
